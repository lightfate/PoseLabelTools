...
if opt.vis_fast:
    from fn import vis_frame_fast as vis_framevis
else:
...
class FrameLoader:
    def __init__(self, frame, batchSize=1, format='yolo', queueSize=50):
        self.img_dir = opt.inputpath
        self.frame = frame
        self.transform = transforms.Compose([
            transforms.ToTensor(),
...
def conserv_result_format_to_list(result):
    #conserv result from tensor to list and add the 18th point
    resultNew = []
    for human in result:
        kp_preds = human['keypoints']
        kp_scores = human['kp_score']
        kp_preds = torch.cat((kp_preds, torch.unsqueeze((kp_preds[5,:]+kp_preds[6,:])/2,0)))#左右肩膀中间位置，非预测得到，取左右平均值
        kp_scores = torch.cat((kp_scores, torch.unsqueeze((kp_scores[5,:]+kp_scores[6,:])/2,0)))
        humanNew = {}
        humanNew['keypoints']=kp_preds.numpy().tolist()
        humanNew['kp_score']=kp_scores.numpy().tolist()
        humanNew['proposal_score']=human['proposal_score'].numpy().tolist()
        resultNew.append(humanNew)
    return resultNew

class FrameProcessor:
    def __init__(self, frame, det_model_loader, pose_model_loader, format='yolo'):
        self.frame = frame
        self.transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
        ])
        self.det_model = det_model_loader.det_model
        self.det_inp_dim = det_model_loader.det_inp_dim
        self.pose_model = pose_model_loader.pose_model
    ...
    def human_detect(self):
        ...
        with torch.no_grad():
            ...
            if isinstance(dets, int) or dets.shape[0] == 0:#no dets
                self.human_detect_result = (None, orig_img[0], None, None, None, None)
            ...
            if isinstance(boxes, int) or boxes.shape[0] == 0:
                self.human_detect_result = (None, orig_img[0], boxes, scores, None, None)
                #print('seection3')
                return
            ...
            self.human_detect_result = (inps, orig_img[0], boxes, scores, pt1, pt2)
            #print('seection4')
            return

    def gen_pose(self):
        with torch.no_grad():
            (inps, orig_img, boxes, scores, pt1, pt2) = self.human_detect_result
            ...
            if not self.Q.empty():
                ...
                if boxes is None:
                    ...
                    if opt.save_img or opt.save_video or opt.vis:
                        ...
                        print(orig_img.shape, img.shape,'img.shape')
                        if opt.vis:
                            cv2.imshow("AlphaPose Demo", img)
                            cv2.waitKey(30)
                        if opt.save_img:
                            print(os.path.join(opt.outputpath, 'vis', im_name))
                            cv2.imwrite(os.path.join(opt.outputpath, 'vis', im_name), img)
                        if opt.save_video:
                            self.stream.write(img)
            else:
